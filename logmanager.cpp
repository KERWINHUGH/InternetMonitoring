 ...
 * 2. 是否包含非法字符（空格、分号等）
 */
bool LogManager::checkInputValid(const QString& user, const QString& pwd, QString& errMsg)
{
    // 检查是否为空
 ...
 * 4. 确定用户角色
 */
bool LogManager::checkUser(const QString& user, const QString& pwd, bool& isAdmin, QString& errMsg)
{
 ...
 * 3. 文件格式：用户名,密码,角色
 */
bool LogManager::registerUser(const QString& user, const QString& pwd, bool isAdmin, QString& errMsg)
{
    QFile file("D:/code/QT/project/InternetMonitoring/users.txt");                       // 用户数据文件
 ...
 * 4. 发送相应的信号
 */
void LogManager::handleLogin(const QString& user, const QString& pwd)
{
    QString errMsg;
    bool isAdmin = false;
 ...
 * 3. 发送相应的信号
 */
void LogManager::handleRegister(const QString& user, const QString& pwd, bool isAdmin)
{
    QString errMsg;
 ...
 * @brief 重置登录尝试次数
 */
void LogManager::resetLoginAttempts()
{
    loginAttempts = 0;
}
 ...
 * @return 登录尝试次数
 */
int LogManager::getLoginAttempts() const
{
